...
class LeaveUpdateFragmentv2 : Fragment() {


    //Data
    private lateinit var Leaves: List<Leave>
    private lateinit var pagination: Pagination
    private lateinit var leaveAdjustmentAdapter: LeaveAdjustmentAdapter
    ...
    private lateinit var leaveUpdateViewModel: LeaveUpdateViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        leaveUpdateViewModel = ViewModelProviders.of(this).get(LeaveUpdateViewModel::class.java)
    }
    ...
    private fun setListeners() {
        leaveUpdateViewModel.leaves.observe(this, Observer<List<Leave>> { it ->
            ...
            if (it != null) {
                ...
                if (it.size > 0) {
                    ...
                    leaveAdjustmentAdapter = LeaveAdjustmentAdapter(it)
                    Collections.reverse(Leaves)
                    rv_list.adapter = leaveAdjustmentAdapter
                    leaveAdjustmentAdapter.notifyDataSetChanged()
        ...
        leaveUpdateViewModel.pagination.observe(this, Observer<Pagination> { it ->
            ...
            if (it != null) {
                ...
                if (pagination.prev_page_url != "null" && pagination.next_page_url != "null") {
                    prev_button.setOnClickListener { leaveUpdateViewModel.retrievePaginated(Flag.PREV_PAGE, status, search, show) }
                    next_button.setOnClickListener { leaveUpdateViewModel.retrievePaginated(Flag.NEXT_PAGE, status, search, show) }
    ...
    private fun loadData(status: String, search: String, page: Int, show: Int) {
        loading()
        leaveUpdateViewModel.retrieveAllLeaves(status, search, page, show)
    }
