...
class EDTRUpdateFragmentv2 : Fragment() {


    //Data
    private lateinit var TimesheetAdjustments: List<EDTRAdjustment>
    private lateinit var pagination: Pagination

    private lateinit var EDTRAdapter: EDTRAdapter
    ...
    private lateinit var timesheetAdjustmentViewModel: TimesheetAdjustmentViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        timesheetAdjustmentViewModel = ViewModelProviders.of(this).get(TimesheetAdjustmentViewModel::class.java)
    }
    ...
    private fun setListeners() {
        timesheetAdjustmentViewModel.timesheetAdjustments.observe(this, Observer<List<EDTRAdjustment>> { it ->

            if (it != null) {
                TimesheetAdjustments = it

                if (it.isNotEmpty()) {

                    EDTRAdapter = EDTRAdapter(it)
                    Collections.reverse(TimesheetAdjustments)
                    rv_list.adapter = EDTRAdapter
        ...
        timesheetAdjustmentViewModel.pagination.observe(this, Observer<Pagination> { it ->
            ...
            if (it != null) {
                ...
                if (pagination.prev_page_url != "null" && pagination.next_page_url != "null") {
                    prev_button.setOnClickListener { timesheetAdjustmentViewModel.retrievePaginated(Flag.PREV_PAGE, status, search, show) }
                    next_button.setOnClickListener { timesheetAdjustmentViewModel.retrievePaginated(Flag.NEXT_PAGE, status, search, show) }
    ...
    private fun loadData(status: String, search: String, page: Int, show: Int) {
        loading()
        timesheetAdjustmentViewModel.retrieveAllTimesheetAdjustments(status, search, page, show)
    }
